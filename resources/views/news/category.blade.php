...
@section('content')
    <div class="flex flex-wrap gap-2 mb-8">
        <a href="{{ route('news.index') }}" class="px-4 py-2 bg-white hover:bg-gray-200 text-gray-700 rounded-full text-sm font-medium border border-gray-300 transition">Все</a>
        @foreach($categories as $cat)
            <a href="{{ route('news.category', $cat->slug) }}" class="px-4 py-2 {{ $cat->id === $category->id ? 'bg-blue-900 text-white' : 'bg-white hover:bg-gray-200 text-gray-700 border border-gray-300' }} rounded-full text-sm font-medium transition">
                {{ $cat->name }}
            </a>
        @endforeach
    </div>

    <h1 class="text-3xl font-bold mb-6 text-gray-900">Категория: {{ $category->name }}</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        @empty
            <p class="text-gray-500 col-span-3">В этой категории пока нет новостей.</p>
        @endforelse
    </div>
